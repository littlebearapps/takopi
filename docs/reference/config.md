...
| `voice_transcription_prompt` | string\|null | `null` | ([#691](https://github.com/littlebearapps/untether/issues/691)) Optional vocabulary-bias prompt (≤1000 chars) passed to the transcription `prompt` param — steers the decoder toward domain proper nouns (`"Trello, Untether, Claude Code"`). Effect is model-dependent; keep it to high-frequency nouns (overstuffing can induce hallucinated terms). Unset = parameter omitted. Hot-reloadable. |
