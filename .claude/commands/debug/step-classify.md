...
| **control-channel** | Approval buttons missing / stuck, PTY FD leak, ask-question crash, outline-gate bypass, ExitPlanMode re-issue loop, AskUserQuestion stuck-sequential | grep `control_request`, `control_response`, `_OUTLINE_PENDING`, `_PENDING_ASK_REQUESTS`; check PTY `finally` cleanup; check session-registry cleanup | [`control-channel.md`](../../rules/control-channel.md) |
