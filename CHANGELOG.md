...
- **fix(telegram):** persist chat-mode sessions for private-chat topics. `_chat_session_key()` returned `None` for *every* message carrying a `message_thread_id`, on the assumption that a thread id always means a supergroup forum topic owned by `TopicStateStore`. Telegram's private-chat topics also carry one, and `_topic_key()` declines them whenever `topics.enabled` is false — so both stores passed, no resume token was ever written to `telegram_chat_sessions_state.json`, and every follow-up in a private topic silently started a fresh agent session unless the user replied with an explicit resume. Engine-independent, because it happens before runner dispatch. Private chats now key on `(chat_id, thread_id)` so each topic resumes independently; the main thread keeps `(chat_id, None)` and its existing session, and group/supergroup topics still return `None` so `TopicStateStore` keeps ownership. The two key shapes cannot collide — a chat is either private or a group, so a given `chat_id` only ever uses the thread-scoped or the sender-scoped form — and `ResumeResolver` already reads `topic_key` ahead of `chat_session_key`, so even a hypothetical overlap resolves to the topic store. 8 new tests in `tests/test_stateless_mode.py` [#734](https://github.com/littlebearapps/untether/issues/734)

- **fix(telegram):** HTML-escape agent-authored question text before it reaches Telegram's HTML parser. An `AskUserQuestion` whose text contained a literal tag — in the reported case a `` `<svg>` `` the agent wrote inside a backtick span while asking about chart rendering — was interpolated raw into a `parse_mode="HTML"` message, and Telegram rejected the **entire** request with `400 Bad Request: can't parse entities: Unsupported start tag "svg" at byte offset 88`. Observed on nsd `0.35.5rc4` (pass 4 of the `untether-nsd-lba1` monitor loop): `callback.dispatch command=aq` at `17:08:17`, `telegram.http_error method=editMessageText status=400` one second later, then `transport.edit.failed has_reply_markup=True`. The lost edit is the one carrying the option keyboard, so the cost is not cosmetic — the run becomes unanswerable from Telegram until it is cancelled. Telegram's HTML mode accepts only a small tag whitelist (`b`, `i`, `u`, `s`, `a`, `code`, `pre`, `tg-spoiler`, `blockquote`), and being inside a `<code>` span does not exempt the text. This is the **same class as [#199](https://github.com/littlebearapps/untether/issues/199)** (HTML injection in auth error output), whose fix escaped `commands/auth.py` but never swept the rest of the repo. Fixed at the two `parse_mode="HTML"` call sites in `commands/ask_question.py` via a new `escape_html=` keyword on `format_question_message`, deliberately **opt-in** rather than escaping unconditionally: the same string is also stored as the progress action title by [#709](https://github.com/littlebearapps/untether/issues/709)'s model advance, and that path renders through `render_markdown`, where markdown-it (`html: False`) already neutralises tags — escaping both would double-escape and show the user a literal `&lt;svg&gt;` on the next heartbeat. Only the agent's text is escaped, never the bot-authored `❓ Question N of M:` prefix, and `quote=False` keeps quotation marks literal (they are legal in HTML text content, so escaping them would surface `&quot;` for a merely quoted question). Blast radius audited: of the 28 `parse_mode="HTML"` sites in `src/`, `ask_question.py` is the only one carrying arbitrary agent-authored free text — the progress renderer uses the `entities` contract, not `parse_mode`, and inline-keyboard button labels are not HTML-parsed by Telegram. 6 new tests in `tests/test_ask_user_question.py`, including a raw-by-default regression guard for the double-escape hazard [#713](https://github.com/littlebearapps/untether/issues/713)
- **fix(telegram):** stop the 30s progress heartbeat re-rendering over the in-place `AskUserQuestion` edit. Answering Q1 in a multi-question flow edits the message to show Q2 and its buttons, but the heartbeat re-rendered from the tracker's intercept-time snapshot — restoring Q1's title and Q1's **option labels** over the top while Q2 was the outstanding question, so the user was shown one question and offered another's answers. Found during the QA-3 drive for [#698](https://github.com/littlebearapps/untether/issues/698) on `@untether_dev_bot` (`0.35.5rc2`); pre-existing on the multi-question path and not caused by the #697/#698 fixes under test at the time. Root cause is that an ask-flow control action is advanced by the `aq` callback handler rather than by further engine events, so the progress model had no way to learn the flow had moved. The tracked action is now bound to its tracker at render time (`register_ask_action_model`, keyed by `request_id`, swept by the existing registry TTL so a `ProgressTracker` cannot leak) and the handler advances the **model first**, before the edit the user sees — which makes the next heartbeat a no-op instead of a regression. The final answer clears the keyboard from the model before the teardown `await`, so a heartbeat landing mid-teardown cannot repaint an answered question's buttons; that is the model-side half of #550's keyboard strip and it also narrows #698's race window. 4 new tests in `tests/test_ask_user_question.py` [#709](https://github.com/littlebearapps/untether/issues/709)
- **fix(mac):** give `collect_proc_diag` a macOS backend so post-result liveness gating is no longer inert on the Mac host. Every `/proc`-derived liveness field returned `None` on macOS — `collect_proc_diag()` is Linux-only by construction — so [#655](https://github.com/littlebearapps/untether/issues/655)'s busy-aware limbo grace and [#653](https://github.com/littlebearapps/untether/issues/653)'s assessed-state log level were **permanently disabled** on `mac`, silently: the gates did not error, they just never engaged. Audit-filed from the `untether-nsd-mac` monitor loop against `0.35.5rc1`. Adds a `ps`-based backend (fixed argv, `nosec B603`) covering the TIME parser, process table, tree CPU and dispatch, so the same liveness verdicts are available on both platforms; `_is_alive` now treats `EPERM` as **alive** rather than dead, which is the correct reading of a permission error on a live PID. 12 new tests in `tests/test_proc_diag.py` [#689](https://github.com/littlebearapps/untether/issues/689)
- **fix(drain):** require evidence of a *self*-restart before taking the 10s fast-drain path. [#559](https://github.com/littlebearapps/untether/issues/559)'s single-run fast-drain was selected purely on `active_runs == 1`, without checking whether the restart actually originated from that run — the code computed `origin_matches` and **logged it, then ignored it**. During a fleet rollout that meant an externally-triggered restart destroyed a demonstrably-busy in-flight run after 10s instead of granting it the full 120s grace. Audit-filed on nsd `0.35.5rc1`, severity `major`. The fast path is now **evidence-gated and fail-closed**: it requires either an origin match or a blocking `systemctl`/`launchctl` descendant, using a strict argv match so an unrelated process cannot masquerade as the restarter; anything short of that evidence gets the full grace. Verified live during the rc2 fleet roll. Tests in `tests/test_shutdown.py` (self-restart argv matcher + descendant evidence scan) [#690](https://github.com/littlebearapps/untether/issues/690)
- **fix(drain):** route drain notices to the run's forum topic instead of the channel root. In a topics-enabled supergroup the drain start/timeout notice was posted without a thread ID, so it landed outside the topic carrying the run it was about — the operator watching that topic saw their run die with no explanation. Notices now carry `SendOptions.thread_id` and dedupe on `(channel, thread)` rather than channel alone, so two concurrent runs in different topics each get their own notice and neither is suppressed as a duplicate; the notify send is bounded to 10s so a slow Telegram call cannot itself extend the drain. The issue's second half — a CPU-idle early-exit to stop burning the full 120s on a demonstrably idle drain — is **deferred**, pending a bridge finalisation signal to distinguish "idle because finished" from "idle between turns". 15 tests in `tests/test_drain_notify.py` [#665](https://github.com/littlebearapps/untether/issues/665)
- **feat(voice):** add `voice_transcription_prompt`, a vocabulary-bias hint passed to the STT API. Voice transcription reliably mangled **domain proper nouns** — tool, project and compound technical terms — while transcribing the surrounding sentence cleanly: two consecutive nsd voice messages in a 20-minute window produced "trollo" (Trello), "RepoSide" and "planned ox". These are exactly the words that carry a spoken instruction's *referent*, so mangling them is disproportionately costly. Whisper-compatible APIs accept a `prompt` parameter that biases decoding toward supplied vocabulary; Untether passed none. The key ships with a settings validator, is **omitted entirely when unset** (so no behaviour change for anyone who doesn't set it), is hot-reloadable, and is **never logged** (it can carry private project nouns). Docs: telegram transport reference, `config.md`, FAQ Q9. Superseded in rc4 by [#703](https://github.com/littlebearapps/untether/issues/703), which ships a default so the bias is not inert on hosts that never set it [#691](https://github.com/littlebearapps/untether/issues/691)
- **feat(cost):** warn once per process when spend is neither displayed nor bounded. nsd spent **$49.05 in 70 minutes** across 7 Claude runs with no `[cost_budget]` block, `show_api_cost = false`, and a subscription-only footer — a configuration in which the operator has no way to see or cap what is being spent, and nothing in the logs says so. Adds a one-shot `config.cost_visibility_gap` WARNING emitted when that exact shape is detected, so the always-on `untether-issue-watcher` can surface it without needing a human to notice the absence of a signal. Deliberately one-shot and process-scoped: it is a config-shape diagnostic, not a spend alarm. Extended in rc4 by [#702](https://github.com/littlebearapps/untether/issues/702), which adds the per-run spend signal this warning can only point at. Tests in `tests/test_cost_tracker.py` [#658](https://github.com/littlebearapps/untether/issues/658)
- **fix(claude):** parse the subscription-cap reset time out of the result-error text instead of discarding it. A session-cap throttle on nsd showed the user **"~60 s"** when the real wait was **~33 minutes** — and the authoritative reset time was already present in Untether's own final message, just not in the `RateLimitInfo` event the latch reads. The wait estimate therefore fell back to [#657](https://github.com/littlebearapps/untether/issues/657)'s conservative 60s default and repeated it for the whole throttle, which reads as a stuck run rather than a long wait. The reset clause is now parsed from the result error, latched **per auth namespace** (so a token switch cannot inherit another identity's deadline), and rendered as an **absolute time** on subsequent bare `rate_limit_event`s under `retry_after_source="result_error"`. Complemented in rc4 by [#701](https://github.com/littlebearapps/untether/issues/701), which handles the second cap class that carries no reset time at all [#692](https://github.com/littlebearapps/untether/issues/692)
- **fix(claude):** show the remedy, not a countdown, for **action-based** subscription caps. [#692](https://github.com/littlebearapps/untether/issues/692)'s `result_error` tier harvests a reset time from the run's error text and is verified working, but a second cap class carries no time at all — *"You've reached your Fable 5 limit. Run `/usage-credits` to continue or switch models with `/model`."* — so that tier has nothing to harvest and every bare `rate_limit_event` fell through to [#657](https://github.com/littlebearapps/untether/issues/657)'s 60s default. On nsd (2026-07-27) both classes fired within 42 minutes; the action-based one displayed "~60 s" **eleven times across eleven real minutes** before the run died with a message stating the cap is not time-based at all. Adds a fourth tier below `reset_ts`/`result_error` and above the default: `retry_after_source="action_required"`, latched per auth namespace the same way #692 latches reset times. Recognition deliberately requires **both** halves — the "reached your `<X>` limit" phrasing *and* the `/usage-credits`|`/model` remedy — because the phrasing alone also appears on time-based caps, so either half missing fails closed to #657. The rendered title names the remedy instead of a timer and **hedges** the claim ("may not clear on a timer"): the nsd cap did clear on its own ~15 min later, so a flat denial would be its own inaccuracy. The 60s deadline is still latched internally so `awaiting_rate_limit_retry()` keeps telling the stall detector the session is throttled rather than hung. Latch TTL is 30 min rather than a real deadline (this class has no reset time) and is only read while genuinely throttled, so a generous window costs nothing; `action_model` is logged on `claude.rate_limit_event` so a window of `action_required` is greppable the way `result_error` now is. 7 new tests in `tests/test_claude_runner.py` [#701](https://github.com/littlebearapps/untether/issues/701)
- **feat(cost):** a per-run spend signal that survives an absent `[cost_budget]` block. [#658](https://github.com/littlebearapps/untether/issues/658)'s `config.cost_visibility_gap` shipped in rc2 and works exactly as designed, but the gap it leaves is structural: `check_run_budget()` returns early without a configured budget, so `warn_at_pct` never evaluates and **no alert can fire at any spend level**. nsd's 2026-07-27 window spent **$372 across ~3h and produced one cost log line, at $14.44**. #658's original framing — sustained burn hiding *beneath* a per-run threshold — was overtaken by that window: it contained a single $22.06 run that cleared the $20 outlier bar outright and still surfaced nothing. The absence of a `[cost_budget]` block is precisely what makes any level of spend unreportable. Adds `cost.run_outlier`, deliberately **outside** the `enabled` gate: it fires for any single run above `[cost_budget] warn_run_above_usd` (unset → $20, matching the bar `/monitor` already uses; `0` disables), logs `total`/`threshold`/`budget_configured`/`show_api_cost` so the issue watcher can ingest it, and adds **one** chat line that is *not* gated on `[footer] show_api_cost` — the operator with the most need to know is the one who turned the footer off (`notify_run_outlier = false` keeps the log without the line). The chat line is suppressed when a budget alert already surfaced this run's spend, so a configured budget never produces two. #658 is untouched and stays a once-per-process config-shape diagnostic; fail-open on config errors, matching `_check_cost_budget`'s posture. Docs: `docs/reference/config.md` `[cost_budget]` table + FAQ Q8. 8 new tests in `tests/test_cost_tracker.py` [#702](https://github.com/littlebearapps/untether/issues/702)
- **feat(voice):** ship a default vocabulary bias so [#691](https://github.com/littlebearapps/untether/issues/691) stops being inert. #691 added `voice_transcription_prompt` in rc2 — validator, omitted when unset, hot-reloadable, never logged — and the implementation is correct, but **no host in the fleet sets it**, so the bias was inert everywhere and the mangling #691 documents ("trollo", "RepoSide", "planned ox") continued until someone hand-edited five TOMLs. A shipped fix delivering zero benefit is, from the user's side, indistinguishable from no fix. Takes the issue's option 1 (a built-in default) over option 2 (seed at rollout): it needs no fleet tooling and benefits users who never run `fleet-rollout.sh`. The default is deliberately **product-generic** — engine names plus Untether's own nouns — because the issue's suggested list included fleet hostnames and third-party tools, which are deployment-specific, have no business in a PyPI wheel, and widen the hallucination surface on the short clips #691 itself warns about; what remains is the class the issue identifies as highest-value, the words carrying a spoken instruction's referent ("run it on Codex"). Resolution mirrors `[preamble]`: unset → the shipped default, `""` → omit the parameter entirely (opt out), any other value → itself verbatim, where an override **replaces** rather than merges so the ~224-token Whisper prompt window stays the operator's to spend. That required the setting to become `str | None` rather than `NonEmptyStr | None` — `""` has to survive validation to mean anything — and resolution lives at the transport boundary (`voice.resolve_transcription_prompt`), keeping the settings layer a plain record of what the operator wrote. Docs: telegram transport reference (default listed verbatim + the replace-not-merge rule), `config.md`, FAQ Q9. 5 new tests across `tests/test_telegram_voice.py` and `tests/test_settings.py` [#703](https://github.com/littlebearapps/untether/issues/703)
- **fix(telegram):** bounds-check the `AskUserQuestion` option index before reading the current question. `AskQuestionCommand.handle` read `flow.questions[flow.current_index]` **before** incrementing the index, and the increment was separated from the flow teardown by an `await` — so two callbacks racing that window left the second one indexing past the end of `flow.questions`. The result was an `IndexError` traceback at ERROR level (`callback.failed`, a signature the always-on `untether-issue-watcher` auto-files on) plus a failure toast, for a double-tap whose action had in fact succeeded. Filed from a code read while fixing [#698](https://github.com/littlebearapps/untether/issues/698) — not observed in production, but reachable by any user double-tapping the final option, and #698's fix deliberately did not widen its scope to cover it. An out-of-range index is now treated as the already-answered case, reusing #698's vocabulary (`ask_question.flow_already_answered` at INFO, "Already answered" toast) so both orderings of a double-tap produce the same truthful outcome instead of one succeeding and one raising. 2 new tests in `tests/test_ask_user_question.py` [#710](https://github.com/littlebearapps/untether/issues/710)

- **fix(watchdog):** stop a same-tick event demoting an approval wait back to a `subprocess.liveness_stall` WARN. The #526 discriminator was **positional** — it tested `recent_events[-1] == "control_request"` — so any event arriving alongside the `control_request` flipped a by-design approval wait into a WARNING. Observed live on nsd `0.35.5rc2` (session `b081a873`): an approval keyboard attached at `16:04:10` and was unanswered until `16:19:44`; ten minutes in, the watchdog logged `subprocess.liveness_stall approval_pending=False last_event_type=rate_limit_event` with the `control_request` sitting at index **-2**, displaced by a `rate_limit_event` sharing the same monotonic timestamp. The bridge-side predicate got it right 13s later from the inline-keyboard payload, so Untether held two disagreeing "is an approval pending" assessments on one session. Not exotic: any Claude turn throttled while an approval is outstanding interleaves this way, which is nsd's steady state (plan mode + subscription caps, see #692/#657). Three costs — a by-design wait reported at WARNING into the always-on `untether-issue-watcher` filing pipeline (dampened today only by signature dedup against the pre-existing #533), a **burnt one-shot stall canary** (the WARN branch latches `liveness_warned`, which the approval branch deliberately does not, so a genuine hang *after* the approval could never be reported for the rest of the run), and a latent auto-kill hazard (the WARN branch falls through to the `_stall_auto_kill` check, which a rate-limited process sleeping on its retry backoff with no established sockets would satisfy on all three conditions). Fixed by promoting the watchdog to the **authoritative** signal the bridge already uses: `engine_state.awaiting_user_approval()` → `pending_control_requests_for_session()`, backed by the self-cleaning `_REQUEST_TO_SESSION` registry, reached by the same duck-typing so non-Claude engines degrade to False. The ring buffer is kept as a True-only fallback but is no longer positional — it scans backwards for a `control_request` with no *resolving* event (`control_response`/`user`/`result`) after it, so `rate_limit_event`, `assistant` and `tool:*` frames are transparent. `pending_control_requests_for_session`'s own docstring had already named the ring-buffer heuristic as stale when #495/#499/#500 introduced the registry; this closes the last consumer that hadn't been switched over. 9 new tests in `tests/test_exec_runner.py`, including an end-to-end fake-CLI repro that emits `control_request` then `rate_limit_event` [#697](https://github.com/littlebearapps/untether/issues/697)
- **fix(telegram):** stop telling the user "Selected" for an AskUserQuestion tap that did nothing. #550 strips the final question's inline keyboard so late taps can't fire `ask_question.flow_missing`, but that strip is an **async outbox edit issued after the flow is already torn down** — and a tap already in flight cannot be recalled, so the losing side of the race was unhandled. Observed on nsd `0.35.5rc2` (`nsd-relationships`, session `b081a873`): `control_response.sent` at `16:48:49` completed a 4-answer sequential flow, a tap landed at `16:48:50`, and the handler logged `ask_question.flow_missing action=opt` at WARNING **after** early-answer had already toasted `Selected`. The gap was one second here; because the strip goes through `TelegramOutbox` (coalesced, priority-scheduled, 1 msg/s private / 20-per-60s groups) it widens well past that in a busy forum supergroup carrying concurrent runs, so narrowing the window is not a fix. Answered flows are now remembered briefly (`_ANSWERED_ASK_FLOWS`, 300s TTL, 32-entry cap, channel-scoped so one chat's late tap can't claim another's answer), which lets both the handler and the pre-`handle` early-answer toast tell a late tap (`ask_question.flow_already_answered`, INFO, toast **"Already answered"**) apart from a genuinely unexplained missing flow (WARNING, "No active question" — unchanged, so it stays visible in warn-level triage). Deliberately not reordered to strip-before-teardown: it would only narrow the window while breaking #550's retry-preserving behaviour on a failed send (buttons are kept on failure so the user can retry). The sibling defect on the `claude_control` path is #685 — same shared cause (the early-answer toast is emitted before the handler can know the action is a no-op) and now the same predicate shape to fix it with. 5 new tests in `tests/test_ask_user_question.py` [#698](https://github.com/littlebearapps/untether/issues/698)
...
- **fix(telegram):** stop a stale approval action from swallowing every later approval keyboard. After a run used "Pause & Outline Plan", the next `AskUserQuestion` rendered its question text with **no option buttons** — only `cancel` — leaving the run unanswerable from Telegram until cancelled. Observed live on nsd `0.35.4`: a plan-mode run in a project chat hung ~10 min at `❓ …?` with `render_progress.inline_keyboard_found action_id=claude.discuss_approve.3` repeating across 98 consecutive renders and no second `progress_edits.keyboard_attach` ever firing. Root cause is the synthetic `claude.discuss_approve.N` action never being completed: the hold-open branch returns early and so skipped the `state.request_to_action` mapping the reconcile loop needs, which showed up in production as `control_request.reconciled action_id=None`. That permanently-uncompleted action then broke three things at once — it won `render_progress`'s **oldest-first** keyboard scan, it kept `_has_pending_approval` state alive, and (dominant for the observed symptom) it kept the #163 "outline owns the approval buttons" strip firing via an `any(uncompleted DiscussApproval)` test, reducing every later keyboard to the cancel row. Fixed at all three sites, which now agree on one convention — *the newest uncompleted action carrying an `inline_keyboard` is the one the user must answer*: `render_progress` scans newest-first, the strip check asks the action that actually supplied the rendered keyboard, and the hold-open path registers `request_to_action` for both variants while the reconcile loop also sweeps `request_to_action` keys so the synthetic action is retired once its button is answered. The `da:` escalation button marks itself handled (it never reaches `send_claude_control_response`) via a new shared `mark_request_handled` helper. Diff-preview approvals and a second `ExitPlanMode` were swallowed by the same three arms. Verified live on `@untether_dev_bot` by replaying the incident sequence — approve → Pause & Outline → Approve Plan → `AskUserQuestion` — which now yields `action_id=claude.control.4 buttons=3` + `keyboard_attach keyboard_rows=4`, real option buttons, a routed answer and `ok=True stall_warnings=0`. 7 new tests across `tests/test_telegram_bridge.py`, `tests/test_cooldown_bypass.py` and `tests/test_exec_bridge.py` [#683](https://github.com/littlebearapps/untether/issues/683)
...
- **fix(auto-continue):** make the salvage predicate enforce the exclusion its docstring already claimed. `_should_auto_continue` documented that "a `result` frame would exclude the predicate entirely rather than discriminate", but it never tested for one — it read `last_event_type`, a *running* value recording the last frame seen rather than whether the run reached its result. Those come apart routinely: **106 healthy (`ok=True`, uncancelled) Claude runs on nsd** logged `session.summary last_event_type=user`, which is precisely the value the predicate treats as its salvage trigger, so all seven gates passed on runs that had completed and delivered an answer. The only thing preventing a spurious re-spawn was the caller's `final_delivery["sent"]` check — described in a comment as "belt-and-braces" when it was in fact load-bearing. A salvage on a finished session costs a full extra Claude run (the window that surfaced this contained runs at $3.07–$19.01) and shows the user an unexplained second turn. Fixed by latching `saw_result` on `JsonlStreamState` when a `result` frame is parsed and gating on it — the sound discriminator here, because **neither** upstream defect emits a `result` at all (claude-code#34142 skips the assistant continuation after a tool_result; #30333 never emits ResultMessage with background subagents), so both mitigations are preserved exactly while completed runs are excluded. Follows #502's precedent one layer out: add a signal rather than change an existing one, leaving `last_event_type` and `recent_events` untouched for stall diagnostics. `saw_result` is also logged on `session.summary` so a `last_event_type=user` line is readable as healthy rather than as a stuck-after-tool_result candidate, and the inverted caller comment is corrected. **The mechanism the issue hypothesised — a trailing `tool_result` frame overwriting `last_event_type` after the terminal `result` — was tested and disproven**: the `result` frame's CompletedEvent sets `did_emit_completed`, both `_iter_jsonl_events` overrides break the read loop immediately, and a post-result frame does not even reach `recent_events` (verified against the real `ClaudeRunner` via a new `trailing_user_after_result` fake-CLI scenario). A `user` value on a completed run therefore means the result never landed on that stream object; that residual is filed separately rather than ridden along. 4 new tests across `tests/test_exec_bridge.py` and `tests/test_noop_resume_harness.py` [#716](https://github.com/littlebearapps/untether/issues/716)
- **fix(telegram):** channel-scope `AskUserQuestion` option taps so one chat cannot answer another chat's question. `AskQuestionCommand.handle` resolved the active flow with `get_ask_question_flow()` — no scope — and the resolver returns the **first** flow in the registry when `channel_id` is `None`. With two concurrent ask flows outstanding, a tap in chat B was answered against whichever iterated first, which may be chat A's. Because callback data is **positional** (`aq:opt:N`), nothing failed: the wrong question was silently recorded with the option label at that index, so the user in chat A saw their question answered by someone else's tap while the user in chat B saw theirs unchanged — no warning, no error signature, nothing for the issue watcher to catch. Reachable anywhere concurrent runs are normal: the 6 per-engine dev chats, channelo's forum supergroup topics, or any host with crons firing alongside interactive use. Pre-existing, found by code read while verifying #713/#709/#710 and filed rather than ridden along. [#698](https://github.com/littlebearapps/untether/issues/698) had already channel-scoped the sibling `recently_answered_ask_flow` lookup for exactly this reason ("so one chat's late tap can't claim another's answer"); the live-flow lookup one line earlier was left unscoped. Both lookups in `handle` are now scoped by `ctx.message.channel_id`, and the pre-`handle` early-answer toast is scoped too — it is chosen from the same per-chat registry state, so unscoped it would tell a chat with no outstanding question "Selected" for a tap that does nothing. Threading the chat into that hook required a signature change to an internal duck-typed hook that is **not** part of the `CommandBackend` Protocol, so dispatch calls it through a helper that falls back to the legacy `(args_text)` form on `TypeError` rather than letting an incompatible backend take out the whole callback. 4 new tests in `tests/test_ask_user_question.py` [#715](https://github.com/littlebearapps/untether/issues/715)
- **feat(cost):** give `cost.run_outlier` the shape of the spend, not just its size. [#702](https://github.com/littlebearapps/untether/issues/702)'s per-run signal shipped in rc4 and is verified working — it fired three times in one 20-minute window during the audit that filed this — but the WARNING carried five fields of which four were static config flags, so the only number explaining a figure was the figure itself. Two runs in the **same session** on channelo made the cost concrete: **2 turns / $20.50** and **40 turns / $19.58**. A 2-turn run cost more than a 40-turn one, and they call for opposite operator responses — the first says "this session's context has grown expensive, start a fresh one", the second says "big task, nothing to do" — yet from the log they were indistinguishable. Same pattern on nsd ($25.56 at 40 turns, $24.30 at 27). The data was already in hand: `_check_run_cost_outlier` receives the whole `usage` dict and read exactly one key from it, while `_usage_payload` had already populated `num_turns`, both duration measures and the full token block. Now logged: `num_turns` (the highest-value discriminator), `usd_per_turn` (derived, but the number an operator reads first), `duration_ms`/`duration_api_ms` (separating "slow and expensive" from "fast and expensive"), and `input_tokens`/`output_tokens`/`cache_read_input_tokens`/`cache_creation_input_tokens` (a high cache-read-to-input ratio *is* the context-bloat signature). Fields are emitted only when present and correctly typed, so a non-Claude engine or a result with no token block logs a smaller line rather than a wall of `None`, and a malformed `usage` sub-dict cannot take down the delivery path. The chat notice is deliberately unchanged — this extends #702's stated intent of logging "so the issue watcher can ingest it" to the fields that make an ingested event actionable, the same argument [#695](https://github.com/littlebearapps/untether/issues/695) made one layer up when it added model attribution to `runner.completed`. 5 new tests in `tests/test_cost_tracker.py` [#717](https://github.com/littlebearapps/untether/issues/717)

- **fix(engines):** give AMP's version-refusal its own error hint instead of a misleading generic one. AMP remotely refuses out-of-date clients with `426 ... This version of Amp is no longer supported. Run 'amp update' to continue.` and exits **1**, so the run did surface as an error — but the payload embeds `invalid_request_error`, which matched the pre-existing generic pattern and produced *"Invalid API request. Try updating the engine CLI to the latest version."* Technically adjacent, practically useless: it does not say the client is refused, and `amp update` is the actual remedy. Fixed with a new `--- Engine end-of-life / unsupported client ---` block placed **first** in `_HINT_PATTERNS` — since first match wins, the **ordering** is the fix, not just the patterns: `ineligibletiererror`, `gemini code assist for individuals`, `this version of amp is no longer supported`, plus a generic `no longer supported` fallback for future vendor wordings. Verified A/B on the identical 426 payload against `@untether_dev_bot`: pre-fix message gave the generic hint, post-fix gives *"AMP is refusing this client version. Run amp update to upgrade..."* Also adds a **defensive** guard in `_run_amp_command`, promoting a zero exit to `1` when stderr carries a fatal marker, keyed on stderr content rather than empty stdout so a genuine no-output success (`threads archive`) is unaffected. That guard is belt-and-braces, not a reproduced bug: measured 2026-07-30, `amp threads list` exits 0 on success / 1 on failure and — being a local command — never hits the server-side version gate, so `/threads` keeps working while `amp -x` is refused. **Correction to an earlier draft of this entry:** it claimed both CLIs exit `rc=0` and that refused runs reached the user as empty answers. Both CLIs exit **1**; the original readings were taken through a shell pipe, so `$?` was `head`/`tail`'s status. The Gemini half is separately ineffective — under Untether that subprocess hangs rather than exiting, so the run stalls to the watchdog auto-cancel and the new Gemini hint never fires. Pre-existing, filed as [#724](https://github.com/littlebearapps/untether/issues/724), not fixed (deprecated engine). 6 new tests in `tests/test_error_hints.py` (including an explicit assertion that the AMP hint outranks the generic pattern) and 5 in `tests/test_threads_command.py` [#721](https://github.com/littlebearapps/untether/issues/721)

### breaking

- **breaking(claude):** Untether's `auto` permission mode is renamed **`plan-auto`**, and `auto` now passes through to Claude Code unmodified. Untether's `auto` was never a CLI mode: `build_args` rewrote it to `--permission-mode plan` and set an internal flag that rubber-stamped the `ExitPlanMode` control request. Claude Code **2.1.228** ships its own `auto` — a classifier-gated mode with 17 allow / 65 soft-deny / 1 hard-deny rules — so ours shadowed it and made it unreachable: anyone copying `auto` out of the Claude Code docs into `untether.toml` got plan-mode-plus-rubber-stamp instead, with no warning. The timing is not incidental — **auto mode becomes Claude Code's default for new Pro/Max/Team sessions on 2026-08-14**, two days after this was found. Only `plan-auto` is translated now (→ CLI `plan`); `default`, `manual`, `plan`, `auto`, `acceptEdits`, `dontAsk` and `bypassPermissions` all reach the binary verbatim. **Migration is split by source, deliberately:** stored chat prefs are rewritten `auto` → `plan-auto` **exactly once**, at first load, guarded by a new `permission_mode_migrated` flag in `telegram_chat_prefs_state.json` — those values were only ever written by Untether's own `/planmode` and `/config` buttons and so unambiguously meant the sugar. The migration is Claude-only, since `auto` is a legitimate and differently-meaning value for Codex. The one-shot guard is load-bearing and was added after integration testing caught the first attempt: a per-read rewrite is correct for legacy values but wrong the moment `auto` becomes a value the user can *choose*, and on `@untether_dev_bot` pressing **Auto** stored `auto` and then read it straight back as `plan-auto`, spawning `--permission-mode plan` — the new mode was unreachable through its own UI. The flag is a field rather than a `STATE_VERSION` bump because `JsonStateStore` *discards* state on a version mismatch, so bumping would have wiped every chat's prefs to migrate one value. Hand-authored TOML is **not** rewritten: there `auto` now means the CLI's auto mode, and the runner logs a one-shot `claude.permission_mode.auto_semantics_changed` WARN naming `plan-auto` as the way back. That is a real behaviour change for unattended crons that set `auto` — they lose the plan ceremony and gain per-action classification with a hard exfiltration deny, which is why this is filed as breaking rather than a fix. The load-bearing question was whether upstream `auto` swallows the control channel and costs Untether its interactivity; probed against the exact argv Untether spawns on CLI 2.1.228, it does not — `AskUserQuestion` still raises a `can_use_tool` control_request under `auto`, so ask-mode buttons keep working, and classifier fallback after repeated blocks routes through `--permission-prompt-tool stdio` as an ordinary Telegram approval. `diff_preview` is inert under `auto`, but it has been inert under `acceptEdits` since it shipped — a pre-existing property of any non-prompting mode, not a new regression. `auto` deliberately does **not** arm the ExitPlanMode rubber stamp: that mode has no plan gate, and arming it would recreate the blanket downstream bypass tracked by [#383](https://github.com/littlebearapps/untether/issues/383) on a mode where it means far more. `/config → Plan mode` is retitled **Permission mode** and gains a `plan-auto` button (`config:pm:pa`) alongside `auto`. Mode table and the full `--permission-prompt-tool` interaction documented in `docs/reference/runners/claude/runner.md`; guardrails in `.claude/rules/control-channel.md` [#741](https://github.com/littlebearapps/untether/issues/741)
...
- **obs(bridge):** log the resolved model on `runner.completed` and `session.summary`. The model lived only in `StartedEvent.meta`, consumed for rendering and never logged at any level — so verifying [#688](https://github.com/littlebearapps/untether/issues/688) on `0.35.5rc2` had to fall back to reading the rendered footer out of Telegram, a route available to an interactive audit but not to the always-on log-only `untether-issue-watcher`. Both halves are now emitted: `model` (the raw ID from the engine) and `model_display` (taken from `_short_model_name` itself, not re-derived, so the logged string is literally what the footer renders). The pair is what makes a shortener regression self-evident from logs alone — `model="claude-opus-5[1m]" model_display="opus"` would have shown #688's worst arm, the silently-dropped 1M-context marker, without anyone reading a chat. Keys are omitted rather than logged as `None` when the engine hasn't reported a model yet, since some ship it late (pi sends it from `message_end` via a supplementary `StartedEvent`); an absent key reads as "not reported" where `None` reads as "reported as nothing". Secondary benefit: per-run model attribution makes cost triage queryable — `total_cost_usd` was previously logged with no record of which model produced it. 5 new tests in `tests/test_exec_bridge.py` [#695](https://github.com/littlebearapps/untether/issues/695)
- **obs(claude):** measure `pending_requests`/`pending_asks` on the pre-result `post_result_idle.tick` instead of hardcoding `0`. The `armed_at is None` branch emitted both as literals while the post-result branch computed them for real — same event name, same field names, one branch measured and one stubbed. Live on nsd `0.35.5rc2` (session `b081a873`) a run with a visibly-pending 4-button approval keyboard logged `pending_asks=0 pending_requests=0` every 30s, which has the shape of a presenter/runner desync and cost a source read to clear as benign. The lookup is the same two comprehensions `_maybe_cancel_pre_result_silence` already runs in the same tick, so the cost is one dict scan per 30s. This also yields the signal that was absent entirely: `claude.pre_result_silence.suppressed` only fires once the [#592](https://github.com/littlebearapps/untether/issues/592) threshold is crossed (count since the rc2 restart: 0), so for the whole approval wait there was no log evidence of the pending request at all — a pre-result tick reading `pending_asks=1` is now a positive, greppable "this run is waiting on the user" marker, and an operator can answer "is anything waiting on me?" from logs alone. Complements [#526](https://github.com/littlebearapps/untether/issues/526)/[#527](https://github.com/littlebearapps/untether/issues/527) on the log side. 2 new tests in `tests/test_claude_runner.py` [#696](https://github.com/littlebearapps/untether/issues/696)
- **obs(claude):** emit one `claude.post_result_idle.subcountdown_exit` line per post-result subcountdown, regardless of duration. The gate computes `cpu_active`/`tree_active` every 5s poll but only surfaced them via the ~30s throttled `subcountdown_tick` ([#650](https://github.com/littlebearapps/untether/issues/650)) or the one-shot `limbo_detected` — so a healthy fast reap cleared the loop before either fired and the verdict was computed, used and discarded. Three subcountdowns on nsd since the rc2 restart (15s / 20s / 10s) each evaluated it several times and logged nothing; `cpu_active=` occurrences from this gate across six audit passes of real production traffic: zero. That blocked [#689](https://github.com/littlebearapps/untether/issues/689)'s Linux-collateral verification for 7 of 12 passes — the evidence that could be gathered came from a different probe site (`runner.py`'s `_watchdog_loop`), which shares the primitives but not the gate logic. The terminal line carries the last computed verdict plus `polls`, `elapsed_s`, `live_background_work`, `in_limbo`, `limbo_grace_applied` and `exit_reason`, and is emitted from a `finally` so it also covers the cancellation path. Strictly less volume than the tick on any long countdown, and #650's blackout fix is untouched (the 30s tick stays; this adds a terminal line). The cheaper alternative — logging the first tick on entry — was rejected because the entry sample always has `prev_diag=None` and so reports `cpu_active=None`, which is exactly the ambiguity #689 is about. 2 new tests in `tests/test_claude_runner.py` [#699](https://github.com/littlebearapps/untether/issues/699)
- **obs:** the three entries above close the log-observability cluster tracked in [#700](https://github.com/littlebearapps/untether/issues/700) — each was discovered because it obstructed the verification of a fix shipping in the same batch, not by reading code looking for missing log lines [#700](https://github.com/littlebearapps/untether/issues/700)
- **deprecate(gemini):** the `gemini` engine is deprecated; removal is targeted for **0.36.0**. Google ended Gemini CLI support for individual and free accounts on **2026-06-18**, directing users to [Antigravity CLI](https://antigravity.google). Verified on lba-1 (gemini v0.40.1, 2026-07-30): an individual account now hard-fails with `IneligibleTierError: This client is no longer supported for Gemini Code Assist for individuals`, `reasonCode: 'UNSUPPORTED_CLIENT'`. So this is not pre-emptive labelling — the documentation was **factually wrong** for the tier most users are on. Enterprise / Google Cloud licences may still work, but Untether no longer verifies this, which is why the copy says "deprecated, enterprise unverified" rather than "dead". The engine stays registered and runnable: nothing is removed, no entry point is unregistered, and no config key changes — removing it would turn a previously-valid `default_engine` into a startup `ConfigError` (`runtime_loader.resolve_default_engine`), a breaking change this release deliberately does not make. Antigravity CLI is tracked as a **separate engine** ([#558](https://github.com/littlebearapps/untether/issues/558), v0.36.0), not a `gemini` rename — its auth, flags and session semantics all differ, so aliasing the old id onto it would silently change a user's provider and billing [#720](https://github.com/littlebearapps/untether/issues/720)
- **deprecate(amp):** the `amp` engine is deprecated; removal is targeted for **0.36.0**, together with the AMP-only `/threads` command. This re-scopes [#458](https://github.com/littlebearapps/untether/issues/458) from "maintenance-only" — that posture promised bug fixes, regression fixes, cross-engine sweep coverage and a required-green `test_amp_runner.py`, i.e. exactly the costs the decision was meant to stop paying. This is a decision about **Untether's integration, not about AMP itself**; nothing upstream deprecated AMP. New evidence sharpened the case: AMP remotely refuses clients it considers out of date, and a build from 2026-05-04 was already hard-refused with `426 This version of Amp is no longer supported. Run 'amp update' to continue.` — an unbounded upgrade treadmill on a cadence Untether doesn't track, for an integration with no observed usage. The copy therefore does **not** promise it keeps working [#458](https://github.com/littlebearapps/untether/issues/458)
- **changes(release):** the integration-test Tier 1 matrix drops from 6 engines to the **4 supported** ones. Neither deprecated engine can pass U1 on the dev host, so requiring them would leave every release either overstating its coverage or blocked on a dead CLI. Applied consistently across `docs/reference/integration-testing.md`, `.claude/rules/release-discipline.md`, `.claude/rules/testing-conventions.md`, `.claude/skills/release-coordination/SKILL.md` and `.claude/commands/debug/step-verify.md`; the two chats and test projects stay in place for opt-in spot checks. Paired with a new **sweep-exemption rule** in `.claude/rules/runner-development.md`: when a cross-engine sweep breaks `gemini` or `amp`, `xfail`/`skip` the test rather than fix the runner. That rule is what actually stops the maintenance cost — dormant runner code is free per release, whereas a required-green test drags every sweep repair back onto the critical path. Their tests are deliberately **not** deleted before the removal release: they run against fake CLIs, cost nothing, and dropping ~100 tests would flatter the 80% coverage gate while reducing compatibility coverage [#720](https://github.com/littlebearapps/untether/issues/720)
- **changes(config):** `/config → Engine & model` marks deprecated engines with a `⚠️` glyph and an explanatory line. Driven by a new `DEPRECATED_ENGINES` frozenset in `ids.py` — deliberately a simple id set rather than an `EngineBackend.status` field, since the richer registry metadata lands in 0.36.0 where it's needed to render a supported `antigravity` beside a retired `gemini`. Both engines remain **selectable**, and the set is **not a blacklist**: a third-party entry point supplying `gemini` or `amp` is still honoured, because this drives presentation only [#720](https://github.com/littlebearapps/untether/issues/720)
- **changes(config):** `/config`'s Claude section is retitled **Permission mode** on both the home page and the sub-page — with Claude Code's own `auto` in the list, "Plan mode" no longer describes what the setting controls. The home-page row reads `Permission mode: plan-auto · auto-approve plans`. Verified rendering live on `@untether_dev_bot`. The onboarding wizard and shipped config templates write no `permission_mode`, so fresh installs are unaffected by the rename; `/model` and `/reasoning` only pass the stored value through and needed no change. `manual`, `dontAsk`, `default` and `bypassPermissions` remain deliberately config-only — they are reachable from `untether.toml` and validated there, but not surfaced as buttons [#741](https://github.com/littlebearapps/untether/issues/741)
...
- **docs:** stop advertising six supported engines. README loses Gemini and Amp from the hero line and gains a `Deprecated engines` section plus a support-status row in the compatibility matrix (the existing ticks are kept but footnoted as "what the integration does today, not a support commitment"); both `docs/reference/runners/{gemini,amp}/runner.md` gain status banners; `docs/reference/runners/index.md` gains a deprecated section. FAQ Q3 (engine list) and Q4 (auth model) updated — both are MUST-update triggers under `.claude/rules/help-faq.md`, and Q4 now states explicitly that Untether never silently reroutes a run to a different provider when an explicitly selected engine is unavailable. `switch-engines`, `troubleshooting` and `install` guides updated, and the five AI context files (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.codex/AGENTS.md`, `.github/copilot-instructions.md`) brought into agreement per `.claude/rules/context-quality.md`. `ROADMAP.md` records both the retirement and the incoming Antigravity engine. Historical CHANGELOG sections and old plans that mention these engines are left untouched — they were accurate for their period [#720](https://github.com/littlebearapps/untether/issues/720)
