...
- **fix(engines):** surface the fatal errors that Gemini and AMP print at `rc=0`. Both deprecated CLIs exit **zero** while writing a terminal error to stderr — Gemini `IneligibleTierError: This client is no longer supported ...`, AMP `426 ... This version of Amp is no longer supported. Run 'amp update' to continue.` — so a refused run reached the user as an empty answer rather than a failure. Two distinct defects. **(1) No error hint matched either signature**, and worse, AMP's payload embeds `invalid_request_error`, which matched the pre-existing generic pattern and produced the actively misleading *"Invalid API request. Try updating the engine CLI to the latest version."* Fixed with a new `--- Engine end-of-life / unsupported client ---` block placed **first** in `_HINT_PATTERNS` (first match wins, so ordering is the fix, not just the patterns): `ineligibletiererror`, `gemini code assist for individuals`, `this version of amp is no longer supported`, plus a generic `no longer supported` fallback for future vendor wordings. **(2) `/threads` rendered an AMP refusal as "No AMP threads found"** — `_run_amp_command` returned AMP's raw exit code and all three callers branch only on `rc != 0`, so with `rc=0` and empty stdout `_parse_thread_table("")` yielded `[]` and the empty-state message told the user they had no threads while AMP was refusing the client. `_run_amp_command` now promotes a zero exit to `1` when stderr carries a fatal marker (`no longer supported`, `unexpected error inside amp`, `error:`); keyed on stderr **content** rather than empty stdout, so a genuine no-output success like `threads archive` still reports `rc=0`. New troubleshooting section documents the direct-CLI repro for both. A fully generic rule — any engine exiting without a terminal/result event is a failure regardless of exit code — is the right long-term shape but spans `runner.py` and `stream_end_events` for all six runners, so it is noted on the issue rather than bundled into a patch. 6 new tests in `tests/test_error_hints.py` (including an explicit assertion that the AMP hint outranks the generic pattern) and 5 in `tests/test_threads_command.py` [#721](https://github.com/littlebearapps/untether/issues/721)
